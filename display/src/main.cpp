...
void updateMachineStatus(int machine, uint8_t status) {
  ...
  switch (status) {
    case STATUS_UP:
      lcd.string(120, top, 240, bottom, GREEN, FONT3, FILL, "UP");
      break;
    case STATUS_DOWN:
      lcd.string(120, top, 240, bottom, RED, FONT3, FILL, "DOWN");
      break;
  }
}
...
void drawHome() {
  ...
  lcd.string(15, 30, 240, 320, WHITE, FONT6, UNFILL, "Micro-Cluster");

  for (int i = 0; i < MACHINES; i++)
    updateMachineStatus(i, machine_status[i]);
}
...
void loop() {
  if (millis() - last_refresh >= REFRESH_INTERVAL) {
    ...
    if (millis() > 5000 && millis() < 7000)
      updateMachineStatus(1, STATUS_UP);
    if (millis() > 6000 && millis() < 8000)
      updateMachineStatus(0, STATUS_UP);
    if (millis() > 7000 && millis() < 9000)
      updateMachineStatus(2, STATUS_UP);
#endif
  }
